...
| --- | --- | --- |
| `test.yml` | `workflow_call` (reusable) | **Leg 1 (deterministic, no Godot binary):** build the source-only package, run pure-managed unit tests, `dotnet pack`, assert the nupkg is source-only with no GodotSharp dep, then build the **testbed consuming the local package** (proves the source-injection recipe). **Leg 2 (live E2E):** install the core addon, build + boot headless Godot via the **safe three-process pattern** (`--import` → `dotnet build` → `--editor` clean-boot assertion), then start the addon-pinned `gamedev-mcp-server` (auth=none) + boot a **connected** headless editor (`Custom` mode) and call **each tool** via `godot-cli run-tool`, asserting success. The per-tool list is data-driven (`tests/e2e/e2e-tools.json`, run by `tests/e2e/run_tool_e2e.py`); a tool that throws turns the leg red (no `continue-on-error`). |
